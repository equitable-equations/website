...
author:
  - name: Andrew Gard
date: 2026-08-24
categories: [regex, stringr, R-bloggers] 
...
Searching and manipulating strings (those things that make up character vectors) is easy as long as you only need to match specific sequences of letters and numbers. To match patterns rather than specific substrings or to search symbols, you'll need to use *regular expressions* (*regex* for short).

Throughout this post, I'll make use three `stringr` functions: `str_subset`, `str_detect`, and `str_extract`, and a subset of the `sentences` vector.
...
In order to match these metacharacters, precede them with two slashes, like so:
